# Super Awesome EvaGrbl python flash script.
# 
...
# script. alternatively you can create a Evaapp.conf file with the port as 
# the first line. Also on OSX and Linux this script usually finds the right port
# automatically.


# (3)
# Compile EvaGrbl and load it to an Arduino Uno via USB. In the Teminal/Command Line
...
def build():
    ...
    BUILDNAME = "EvaGrbl"
    OBJECTS  = ["coolant_control",'eeprom','gcode','limits','main','motion_control','nuts_bolts','planner','print','protocol','report','serial',"settings", "spindle_control", "stepper","sync_control"]
    ...
    if os.path.isfile(file_abs):
        os.remove(file_abs)
    # file_abs = os.path.join(current_dir, 'EvaGrbl.hex')
    # if os.path.isfile(file_abs):
    #     os.remove(file_abs)
