...
;
	...
	public void run(String ignored) {
		...
		for (int i=0; i<commands.length; ++i) {
			...
			if(ca.menuItem != null){
				try{	
					listLabelToCommand.put(makeListLabel(commands[i], ca, true), ca.menuItem.getActionCommand());
					listLabelToCommand.put(makeListLabel(commands[i], ca, false), ca.menuItem.getActionCommand());
					}
				catch(Exception e){
					;
				}
