...
public class Toolbar extends Canvas implements MouseListener, MouseMotionListener, ItemListener, ActionListener {
	...
	private void showMessage(int tool) {
		...
		switch (tool) {
			case RECTANGLE:
				if (roundRectMode)
					IJ.showStatus(translation.tr("Rectangular or *rounded rectangular* selections"+hint));
				else
					IJ.showStatus(translation.tr("*Rectangular* or rounded rectangular selections"+hint));
				return;
			case OVAL:
				if (ovalType==BRUSH_ROI)
					IJ.showStatus(translation.tr("Oval, elliptical or *brush* selections"+hint));
				else if (ovalType==ELLIPSE_ROI)
					IJ.showStatus(translation.tr("Oval, *elliptical* or brush selections"+hint));
				else
					IJ.showStatus(translation.tr("*Oval*, elliptical or brush selections"+hint));
				return;
			case POLYGON:
				IJ.showStatus(translation.tr("Polygon selections"));
				return;
			case FREEROI:
				IJ.showStatus(translation.tr("Freehand selections"));
				return;
			case LINE:
				if (arrowMode)
					IJ.showStatus(translation.tr("Straight, segmented or freehand lines, or *arrows*"+hint));
				else
					IJ.showStatus(translation.tr("*Straight*, segmented or freehand lines, or arrows"+hint));
				return;
			case POLYLINE:
			...
			case POINT:
				if (multiPointMode)
					IJ.showStatus(translation.tr("Point or *multi-point* selections"+hint));
				else
					IJ.showStatus(translation.tr("*Point* or multi-point selections"+hint));
				return;
			case WAND:
				IJ.showStatus(translation.tr("Wand (tracing) tool"));
				return;
			case TEXT:
				IJ.showStatus(translation.tr("Text tool"));
				TextRoi.recordSetFont();
				return;
			case MAGNIFIER:
				IJ.showStatus(translation.tr("Magnifying glass (or use \"+\" and \"-\" keys)"));
				return;
			case HAND:
				IJ.showStatus(translation.tr("Scrolling tool (or press space bar and drag)"));
				return;
			case DROPPER:
				IJ.showStatus(translation.tr("Color picker (" + foregroundColor.getRed() + ","
				+ foregroundColor.getGreen() + "," + foregroundColor.getBlue() + ")"));
				return;
			case ANGLE:
				IJ.showStatus(translation.tr("Angle tool"));
				return;
			default:
				IJ.showStatus(translation.tr("ImageJ "+IJ.getVersion()+" / Java "+System.getProperty("java.version")+(IJ.is64Bit()?" (64-bit)":" (32-bit)")));
				return;
		}
	}
