...
public class HistogramWindow extends ImageWindow implements Measurements, ActionListener, ClipboardOwner,
	...
	void drawText(ImageProcessor ip, int x, int y, boolean fixedRange) {
		...
		ip.drawString(translation.tr("Count: ") + count, col1, row1);
		ip.drawString(translation.tr("Mean: ") + d2s(stats.mean), col1, row2);
		ip.drawString(translation.tr("StdDev: ") + d2s(stats.stdDev), col1, row3);
		ip.drawString(translation.tr("Mode: ") + d2s(stats.dmode) + modeCount, col2, row3);
		ip.drawString(translation.tr("Min: ") + d2s(stats.min), col2, row1);
		ip.drawString(translation.tr("Max: ") + d2s(stats.max), col2, row2);
		
		if (showBins) {
			ip.drawString(translation.tr("Bins: ") + d2s(stats.nBins), col1, row4);
			ip.drawString(translation.tr("Bin Width: ") + d2s(binWidth), col2, row4);
		}
	}
