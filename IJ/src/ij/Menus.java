...
public class Menus {
	...
	static void addItem(Menu menu, String label, int shortcut, boolean shift) {
		...
		MenuItem item;
		if (shortcut==0){
			item = new MenuItem(translation.tr(label));
		}
		else {
			if (shift) {
				item = new MenuItem(translation.tr(label), new MenuShortcut(shortcut, true));
				shortcuts.put(new Integer(shortcut+200),label);
			} else {
				item = new MenuItem(translation.tr(label), new MenuShortcut(shortcut));
	...
	CheckboxMenuItem addCheckboxItem(Menu menu, String label, String className) {
		pluginsTable.put(label, className);
		nPlugins++;
		CheckboxMenuItem item = new CheckboxMenuItem(translation.tr(label));
		item.setActionCommand(label);
		menu.add(item);
	...
	static Menu addSubMenu(Menu menu, String name) {
		...
		int index;
 		Menu submenu=new Menu(translation.tr(name).replace('_', ' '));
 		submenu.setActionCommand(name);
		index = key.indexOf(' ');
		if (index>0)
		...
		if (result == null) {
			...
			if (offset < 0) {
				...
				if (menuName.equals("Window"))
					...
					pluginsMenu = result;
				result.setLabel(translation.tr(menuName));
			} else {
				...
				result.setLabel(translation.tr(menuItemName));
	...
	void installPopupMenu(ImageJ ij) {
		...
		while (true) {
			...
			if (s.equals("-"))
				...
				mi = new MenuItem(translation.tr(s));
